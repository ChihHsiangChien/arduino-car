 ...
 * 
 * 教學目標：
 * 1. 學習「開迴路控制 (Open-Loop Control)」：
 *    - 定義：系統只根據預設指令動作，不檢查實際結果是否正確。
 *    - 舉例：像是在跑步機上跑步，你跑了10分鐘，但不知道實際跑了多遠。
 *    - 在此程式中：我們叫車子轉彎 450ms，假設它會轉 90 度，但如果地毯太滑或電池沒電，它可能只轉了 80 度或轉過頭。
 *      這就是開迴路控制的缺點 - 沒有回饋機制修正誤差。
 * 
 * 2. 練習調整參數：因為沒有編碼器 (Encoder) 或陀螺儀來做「閉迴路 (Closed-Loop)」，
 *    你需要不斷測試並手動修改 delay 的時間 (TURN_TIME)，才能讓車子轉出完美的 90 度直角。
 * 
